...
public class PlayState extends GameState{
	...
	public void DefinitionMyWorldMethod() {
		...
		player = createBox(350, 320, 60, 20, true, false);
		player2 = createBox(350, 360, 20, 20, false, false);
		circle1  = createCircle(350, 650, 20, false);
		...
		dDef.frequencyHz =1f; 	// salınım frekansı
		dDef.dampingRatio =1f; // sönümleme oranı
		
		df = (DistanceJoint) world.createJoint(dDef);
	...
	public void HandleInput(float dt) {
		...
		if(Gdx.input.isKeyPressed(Input.Keys.SPACE)) {
			circle1.applyForceToCenter(0, -10,true);
		}
		if(Gdx.input.isKeyPressed(Input.Keys.W)) {
			player2.applyForceToCenter(0, 50,true);
		}
		if(Gdx.input.isKeyJustPressed(Input.Keys.UP)) {
			df.setLength(160/PPM);
		}
